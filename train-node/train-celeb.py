...
def trainfn(model, inputs, labels):
  with tf.GradientTape() as tape:
    ...
    loss = loss_fn(labels, model(inputs, is_training=True))
...
for epoch in range(epochs):
  ...
  testing_loss_at_epoch = np.mean(loss_fn(y_test[:test_batch], DCODNN(x_test[:test_batch], is_training=False).numpy()))
  _ = metric.update_state(y_test[:test_batch], DCODNN(x_test[:test_batch], is_training=False).numpy())
